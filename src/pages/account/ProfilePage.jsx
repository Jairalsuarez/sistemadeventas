...
export default function ProfilePage({ onSave, onUploadAvatar, user }) {
  ...
  return (
    <div className="space-y-6">
      <PageHeader eyebrow="Perfil" title="Mi cuenta" description="Actualiza tu informacion personal y como te ven dentro del negocio." />
      ...
      <SectionBlock description="Tu foto, tu nombre y el rol con el que trabajas cada dia." title="Datos personales">
        <form className="grid gap-5 md:grid-cols-[180px_minmax(0,1fr)]" onSubmit={save}>
          <div className="space-y-3">
            <button
              className="group relative grid h-40 w-40 place-items-center overflow-hidden rounded-lg border border-[#dfe7db] bg-[#f8faf6] text-left dark:border-white/10 dark:bg-[#0d1710]"
            ...
            >
              ...
              <span className="absolute inset-x-3 bottom-3 inline-flex items-center justify-center gap-2 rounded-md bg-white/92 px-3 py-2 text-sm font-medium text-[#183325] opacity-0 transition group-hover:opacity-100 dark:bg-[#122117]/92 dark:text-white">
          ...
          <div className="grid gap-4">
            <label className="grid gap-2 text-sm">
              Nombre
              <input className="rounded-md border border-[#dfe7db] px-3 py-2 dark:border-white/10 dark:bg-[#0d1710]" onChange={(event) => setForm((current) => ({ ...current, nombre: event.target.value }))} value={form.nombre} />
            </label>
            <label className="grid gap-2 text-sm">
              Apellido
              <input className="rounded-md border border-[#dfe7db] px-3 py-2 dark:border-white/10 dark:bg-[#0d1710]" onChange={(event) => setForm((current) => ({ ...current, apellido: event.target.value }))} value={form.apellido} />
            </label>
            <label className="grid gap-2 text-sm">
              Correo
              <input className="rounded-md border border-[#dfe7db] px-3 py-2 dark:border-white/10 dark:bg-[#0d1710]" disabled value={user?.email || ""} />
            </label>
            <label className="grid gap-2 text-sm">
              Telefono
              <input className="rounded-md border border-[#dfe7db] px-3 py-2 dark:border-white/10 dark:bg-[#0d1710]" onChange={(event) => setForm((current) => ({ ...current, telefono: event.target.value }))} placeholder="Ej. 593999999999" value={form.telefono} />
            </label>
            <label className="grid gap-2 text-sm">
              Rol
              <input className="rounded-md border border-[#dfe7db] px-3 py-2 dark:border-white/10 dark:bg-[#0d1710]" disabled value={user?.role || ""} />
            </label>
            ...
            <div>
              <button className="rounded-md bg-[#1f7a3a] px-4 py-2 text-sm font-medium text-white disabled:cursor-not-allowed disabled:opacity-50" disabled={!hasChanges} type="submit">
                Guardar cambios
              </button>
            </div>
          </div>
      ...
      <Modal open={avatarModalOpen} onClose={() => !avatarBusy && setAvatarModalOpen(false)} text="Actualiza o elimina la foto visible en tu perfil." title="Foto de perfil">
        <div className="space-y-4">
          <div className="flex items-center gap-4">
            <div className="grid h-20 w-20 place-items-center overflow-hidden rounded-lg border border-[#dfe7db] bg-[#f8faf6] dark:border-white/10 dark:bg-[#0d1710]">
              {form.avatarUrl ? (
                <img alt={fullName} className="h-full w-full object-cover" src={form.avatarUrl} />
              ) : (
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <label className={`inline-flex cursor-pointer items-center justify-center gap-2 rounded-md bg-[#1f7a3a] px-4 py-3 text-sm font-medium text-white ${avatarBusy ? "pointer-events-none opacity-70" : ""}`}>
          ...
          <div className="flex justify-end">
            <button
              className="rounded-md border border-[#dfe7db] px-4 py-2 text-sm font-medium text-[#183325] dark:border-white/10 dark:text-white disabled:opacity-50"
              disabled={avatarBusy}
