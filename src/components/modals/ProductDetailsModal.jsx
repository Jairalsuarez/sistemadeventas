  ...
  return (
    <Modal closeOnBackdrop open={open} onClose={onClose} text={product?.categoria || ""} title={product?.nombre || "Producto"} variant={variant} wide>
      <div className="grid gap-5 lg:grid-cols-[minmax(320px,1fr)_380px] lg:items-stretch">
        <div className={`overflow-hidden p-4 dark:bg-[#0d1710] ${isPublic ? "rounded-[24px] bg-[radial-gradient(circle_at_top,#eef7ef,#dfeee3)]" : "rounded-xl bg-[#f7faf6]"}`}>
          <div className={`flex h-[520px] items-center justify-center overflow-hidden dark:bg-[#122117] ${isPublic ? "rounded-[20px] bg-white/92" : "rounded-lg bg-white"}`}>
            <img className="h-full w-full object-contain" src={product?.imagen_url} alt={product?.nombre} />
          </div>
        </div>
        <div className={`flex h-[520px] flex-col border p-5 dark:border-white/10 dark:bg-[#122117] ${isPublic ? "rounded-[24px] border-[#dceadf] bg-white/84" : "rounded-xl border-[#e4ece2] bg-white"}`}>
          <div className="space-y-5">
            <div className="space-y-2">
              ...
              <p className="text-sm leading-7 text-[#5b6d61] dark:text-white/68">{product?.descripcion}</p>
            </div>
            ...
            <div className={`border p-4 dark:border-white/10 dark:bg-[#0d1710] ${isPublic ? "rounded-[20px] border-[#edf1ea] bg-[#f7fbf8]" : "rounded-lg border-[#edf1ea] bg-[#fbfcfa]"}`}>
